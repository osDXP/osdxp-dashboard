 ...
 *
 * @see  class-wp-plugin-upgrader.php
 *
 ...
 *
 * It is designed to upgrade/install module from a local zip or uploaded zip file.
 *
 *
 * @see OSDXP_Upgrader
 */
...
require_once OSDXP_DASHBOARD_DIR . 'includes/dependencies/wordpress/class-osdxp-module-upgrader-skin.php';

/** Bulk_Module_Upgrader_Skin class */
require_once OSDXP_DASHBOARD_DIR . 'includes/dependencies/wordpress/class-osdxp-bulk-module-upgrader-skin.php';

/** Module_Installer_Skin class */
require_once OSDXP_DASHBOARD_DIR . 'includes/dependencies/wordpress/class-osdxp-module-installer-skin.php';

class OSDXP_Module_Upgrader extends WP_Upgrader
{
	...
	public function bulk_upgrade($plugins, $args = array())
	{
		...
		foreach ($plugins as $plugin) {
			$this->update_current++;
			$this->skin->module_info = get_plugin_data(WP_PLUGIN_DIR . '/' . $plugin, false, true);
			if (! isset($current->response[ $plugin ])) {
			...
			$this->skin->module_active = is_plugin_active($plugin);
